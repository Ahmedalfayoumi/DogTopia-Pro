...
const SalesPage: React.FC = () => {
  const { items, sales, clients, recordSale, updateSale, deleteSale, currencies, defaultCurrencyId } = useInventory();
          ...
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                {modalMode === 'view' ? '🔍 View Invoice' : modalMode === 'edit' ? '✏️ Edit Invoice' : '📤 Record New Sale'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white disabled:bg-gray-50"
              ...
              <div className="pt-6 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 sticky bottom-0 bg-white pb-2">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                  <button type="button" onClick={closeModal} className="flex-1 md:flex-none px-8 py-3 border-2 border-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition-colors">
                  ...
                  {modalMode !== 'view' && (
                    <button type="submit" className="flex-1 px-12 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-lg transition-all active:scale-[0.98]">
                      {modalMode === 'edit' ? 'Update Invoice' : 'Finalize Sale'}
                    </button>
                  )}
                </div>
              </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Summary of all past customer transactions</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {sales.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-16 text-center text-gray-400 italic">No transactions recorded yet</td></tr>
              ) : (
                sales.map(s => (
