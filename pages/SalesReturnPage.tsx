...
import { Sale, SaleReturn, TransactionItem } from '../types';

const SalesReturnPage: React.FC = () => {
  const { sales, saleReturns, recordSaleReturn, deleteSaleReturn, items, currencies, defaultCurrencyId } = useInventory();
  ...
  const [formData, setFormData] = useState<Omit<SaleReturn, 'id'>>({
    originalSaleId: '',
    clientId: '',
    customerName: '',
  ...
  const handleQtyChange = (index: number, val: string) => {
    ...
    if (qty > maxQty) {
      alert(`Cannot return more than originally sold (${maxQty})`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    recordSaleReturn({ ...formData, items: finalItems });
    closeModal();
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-bold text-gray-800">
                {step === 'select_invoice' ? '🔍 Step 1: Select Original Sale' : '🔄 Step 2: Configure Return'}
              </h2>
            ...
            <div className="flex-grow overflow-y-auto p-6">
              {step === 'select_invoice' ? (
                <div className="space-y-4">
                  <p className="text-sm text-gray-500 mb-4">Please choose the sales invoice containing the items to be returned.</p>
                  <div className="relative mb-4">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">🔍</span>
                    <input 
                      type="text" 
                      placeholder="Search original invoices..." 
                      className="block w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white transition-all text-sm" 
                      value={searchTerm} 
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {filteredSales.length === 0 ? (
                      <div className="text-center py-12 text-gray-400 italic">No sales invoices found.</div>
                    ) : (
                      filteredSales.map(inv => (
                        ...
                        >
                          <div>
                            <div className="font-bold text-gray-800">{inv.id} &bull; {inv.customerName}</div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 flex justify-between items-center">
                    <div>
                      ...
                      <div className="font-bold text-indigo-800">{selectedInvoice?.id} for {selectedInvoice?.customerName}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <input required placeholder="e.g. Defective, change of mind..." className="w-full px-4 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" value={formData.reason} onChange={e => setFormData({...formData, reason: e.target.value})} />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block border-b border-gray-100 pb-2">Items to Return (Stock will increase)</label>
                      ...
                      return (
                        <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                          <div className="md:col-span-6">
                            ...
                            <div className="text-[10px] text-gray-400 uppercase">Sold: {maxQty} &bull; Price: {currency.symbol}{formatAmount(rItem.unitPrice)}</div>
                          </div>
                          ...
                          <div className="md:col-span-3 text-right">
                            <div className="text-[9px] font-bold text-gray-400 uppercase">Refund Value</div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-8 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h2 className="text-3xl font-black text-gray-800 tracking-tight">Sales Returns</h2>
            <p className="text-gray-500 mt-1">Manage stock-in returns from customer sales invoices.</p>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredReturns.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-16 text-center text-gray-400 italic">No sales returns recorded yet.</td></tr>
              ) : (
                filteredReturns.map(ret => (
                  <tr key={ret.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-bold text-gray-500 text-sm">{ret.originalSaleId}</td>
                    <td className="px-6 py-4 font-bold text-gray-800">{ret.customerName}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => { if(confirm('Delete this return record? Warning: This will not automatically reverse stock.')) deleteSaleReturn(ret.id) }} className="p-2 text-gray-300 hover:text-red-600 transition-colors">✕</button>
                    </td>
                  </tr>
                ))
              )}
